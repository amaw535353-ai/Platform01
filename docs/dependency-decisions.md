...
The remediation makes pytest-cov an explicit exact-pinned development dependency and uses a committed uv resolution for frozen synchronization. Pydantic v2 is the strict, extra-forbidden request boundary; FastAPI/Uvicorn provide only local HTTP and health surfaces. No identity, policy-agent, database, or MCP client is represented as live enforcement.
